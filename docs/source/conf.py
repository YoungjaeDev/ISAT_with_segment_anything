...
def _normalize_language_tag(value):
    if not value:
        return 'en'
...
language = _normalize_language_tag(os.environ.get('READTHEDOCS_LANGUAGE', 'en'))
...
def _on_config_inited(app, config):
    config.language = _normalize_language_tag(config.language)

...
class Converter(QThread, ISAT):
    ...
    def load_from_isat(self):
        ...
        self.cates = ()
        json_files = [
            file for file in os.listdir(self.isat_json_root) if file.endswith(".json")
        ]
        ...
        for index, file in enumerate(json_files):
            ...
            self.message.emit(
                index + 1,
                num_json_files,
                "{:>8d}/{:<8d} | Loading ISAT json {}".format(
                    index + 1, num_json_files, file
                ),
            )
            try:
    ...
    def convert_to_isat(self):
        ...
        for index, (name_without_suffix, anno) in enumerate(self.annos.items()):
            ...
            self.message.emit(
                index + 1,
                num_annos,
                "{:>8d}/{:<8d} | Converting to {}".format(
                    index + 1, num_annos, name_without_suffix + "json"
                ),
            )
            try:
                self.save_one_isat_json(
                    anno,
                    os.path.join(
                        self.isat_json_root, "{}.json".format(name_without_suffix)
                    ),
                )
...
class COCOConverter(Converter, COCO):
    ...
    def isat_to_coco(self):
        ...
        # convert to coco
        self.message.emit(
            -1, -1, " " * 18 + "| Saving COCO json to {}.".format(self.coco_json_path)
        )
        try:
...
class YOLOConverter(Converter, YOLO):
    ...
    def run(self):
        if self.yolo2isat:
            if (
                self.yolo_images_root is None
                or self.yolo_txt_root is None
                or self.isat_json_root is None
            ):
                return
            self.yolo_to_isat()
        else:
    ...
    def yolo_to_isat(self):
        ...
        if self.yolo_category_file is not None:
            ...
            with open(self.yolo_category_file, "r") as f:
                ...
                for index, line in enumerate(lines):
                    ...
                    self.message.emit(
                        -1, -1, " " * 18 + "| {} : {}".format(index, line.rstrip("\n"))
                    )
            self.message.emit(-1, -1, " " * 18 + "| Loaded.")
        else:
            class_dict = None
        ...
        for index, img_name in enumerate(img_files):
            ...
            self.message.emit(
                index + 1,
                num_img_files,
                "{:>8d}/{:<8d} | Loading from {}".format(
                    index + 1, num_img_files, name_without_suffix + ".txt"
                ),
            )
            try:
        ...
        for index, (name_without_suffix, anno) in enumerate(self.annos.items()):
            ...
            try:
                self.message.emit(
                    index + 1,
                    num_annos,
                    "{:>8d}/{:<8d} | Save yolo txt to {}".format(
                        index + 1, num_annos, name_without_suffix + ".txt"
                    ),
                )
...
class LABELMEConverter(Converter, LABELME):
    ...
    def labelme_to_isat(self):
        ...
        json_files = [
            file
            for file in os.listdir(self.labelme_json_root)
        ...
        for index, file in enumerate(json_files):
            ...
            self.message.emit(
                ...
                num_json_files,
                "{:>8d}/{:<8d} | Loading from {}".format(
                    index + 1, num_json_files, name_without_suffix + ".json"
                ),
            )
            try:
                anno = self.load_one_labelme_json(
                    os.path.join(self.labelme_json_root, file)
                )
                self.annos[name_without_suffix] = anno
            except Exception as e:
                self.message.emit(-1, -1, " " * 18 + "| Error: {}.".format(e))
    ...
    def isat_to_labelme(self):
        ...
        for index, (name_without_suffix, anno) in enumerate(self.annos.items()):
            ...
            json_path = os.path.join(
                self.labelme_json_root, name_without_suffix + ".json"
            )
            self.message.emit(
                index + 1,
                num_annos,
                "{:>8d}/{:<8d} | Save LABELME json to {}".format(
                    index + 1, num_annos, name_without_suffix + ".json"
                ),
            )
            try:
        ...
        if not self.is_instance and self.use_setting_color:
            if os.path.exists(isat_yaml):
                ...
                self.message.emit(
                    ...
                    " " * 18
                    + "| Warning: {}.".format(
                        "Not found isat.yaml, will use default color."
                    ),
                )
        ...
        for index, (name_without_suffix, anno) in enumerate(self.annos.items()):
            ...
            try:
                self.message.emit(
                    index + 1,
                    num_annos,
                    "{:>8d}/{:<8d} | Save voc png to {}".format(
                        index + 1, num_annos, name_without_suffix + ".png"
                    ),
                )
    ...
    def apply(self):
        ...
        if self.tabWidget.currentWidget() == self.tab_COCO:
            ...
            if self.toolBox_coco.currentWidget() == self.toolbox_item_coco2isat:
                # coco2isat
                if (
                    self.lineEdit_coco2isat_coco_json_path.text()
                    and self.lineEdit_coco2isat_isat_json_root.text()
                ):
                    self.converter = COCOConverter()
                    self.converter.message.connect(self.print_message)
                    self.converter.coco2isat = True
                    self.converter.coco_json_path = (
                        self.lineEdit_coco2isat_coco_json_path.text()
                    )
                    self.converter.isat_json_root = (
                        self.lineEdit_coco2isat_isat_json_root.text()
                    )
                    self.converter.keep_crowd = (
                        self.checkBox_coco2isat_keep_crowd.isChecked()
                    )
                ...
                # isat2coco
                if (
                    self.lineEdit_isat2coco_isat_json_root.text()
                    and self.lineEdit_isat2coco_coco_json_path.text()
                ):
                    self.converter = COCOConverter()
                    self.converter.message.connect(self.print_message)
                    ...
                    self.converter.coco_json_path = (
                        self.lineEdit_isat2coco_coco_json_path.text()
                    )
                    self.converter.keep_crowd = (
                        self.checkBox_isat2coco_keep_crowd.isChecked()
                    )
            ...
            if self.toolBox_yolo.currentWidget() == self.toolbox_item_yolo2isat:
                # yolo2isat
                if (
                    self.lineEdit_yolo2isat_yolo_image_root.text()
                    and self.lineEdit_yolo2isat_yolo_txt_root.text()
                    and self.lineEdit_yolo2isat_isat_json_root.text()
                ):
                    self.converter = YOLOConverter()
                    self.converter.message.connect(self.print_message)
                    self.converter.yolo2isat = True
                    self.converter.yolo_images_root = (
                        self.lineEdit_yolo2isat_yolo_image_root.text()
                    )
                    self.converter.yolo_txt_root = (
                        self.lineEdit_yolo2isat_yolo_txt_root.text()
                    )
                    self.converter.isat_json_root = (
                        self.lineEdit_yolo2isat_isat_json_root.text()
                    )
                    if self.lineEdit_yolo2isat_yolo_cate_path.text():
                        self.converter.yolo_category_file = (
                            self.lineEdit_yolo2isat_yolo_cate_path.text()
                        )
                ...
                if (
                    ...
                    and self.lineEdit_isat2yolo_yolo_txt_root.text()
                ):
                    self.converter = YOLOConverter()
                    self.converter.message.connect(self.print_message)
                    ...
                    self.converter.yolo_txt_root = (
                        self.lineEdit_isat2yolo_yolo_txt_root.text()
                    )
            ...
            if self.toolBox_labelme.currentWidget() == self.toolbox_item_labelme2isat:
                # labelme2isat
                if (
                    self.lineEdit_labelme2isat_labelme_json_root.text()
                    and self.lineEdit_labelme2isat_isat_json_root.text()
                ):
                    self.converter = LABELMEConverter()
                    self.converter.message.connect(self.print_message)
                    self.converter.labelme2isat = True
                    self.converter.labelme_json_root = (
                        self.lineEdit_labelme2isat_labelme_json_root.text()
                    )
                    self.converter.isat_json_root = (
                        self.lineEdit_labelme2isat_isat_json_root.text()
                    )
                ...
                # isat2labelme
                if (
                    self.lineEdit_isat2labelme_isat_json_root.text()
                    and self.lineEdit_isat2labelme_labelme_json_root.text()
                ):
                    self.converter = LABELMEConverter()
                    self.converter.message.connect(self.print_message)
                    ...
                    self.converter.labelme_json_root = (
                        self.lineEdit_isat2labelme_labelme_json_root.text()
                    )
            ...
            # VOC
            if (
                self.lineEdit_isat2voc_isat_json_root.text()
                and self.lineEdit_isat2voc_voc_png_root.text()
            ):
                ...
                self.converter.use_setting_color = (
                    self.checkBox_use_setting_color.isChecked()
                )
                self.checkBox_use_setting_color.setEnabled(
                    not self.checkBox_is_instance.isChecked()
                )
                self.converter.run()
            else:
                QtWidgets.QMessageBox.warning(self, "", "")
            ...
            # VOC_DETECTION
            if (
                self.lineEdit_isat2vocod_isat_json_root.text()
                and self.lineEdit_isat2vocod_voc_xml_root.text()
            ):
                ...
                self.converter.voc_xml_root = (
                    self.lineEdit_isat2vocod_voc_xml_root.text()
                )
                self.converter.run()
            else:
                QtWidgets.QMessageBox.warning(self, "", "")
